...
I ran this daily against a production codebase for several months. My own merged-PR
throughput went up substantially, and the work held up in normal code review.

I'm deliberately not putting a multiplier on that. It's my measurement of my own
output on one codebase, the baseline moved for other reasons too, and a number
precise enough to quote is a number precise enough to argue about. If you want the
detail behind it, ask me and I'll walk you through what I actually tracked.
...
shared/              -- the agent role prompts (AGENTS, marut, azer, opus, jobs, skills)
scripts/             -- liveness check and systemd units
hooks/               -- Claude Code hooks
```

The athanor home (`~/athanor/`) lives outside this repo — it contains all athanor
instances and the goals, opera and trails that run through them. This repo is the
tool: the CLI source and the agent prompts that drive it.

`shared/` is where the interesting reading is. `AGENTS.md` defines the core geas and
vocabulary; `marut.md` and `azer.md` define the supervisor and worker roles.
