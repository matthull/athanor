...
**Status:** Draft
**Created:** 2026-03-21
**Location:** `specs/` (in-repo)
**Repo:** `~/code/whisper` (`github.com/matthull/whisper`)
**Language:** Go
...
- **Work from `~/code/whisper/`** — start Claude Code sessions in this directory. The `cd ~/code/whisper &&` pattern causes permission prompts on every command.
- **Gastown reference code** is at `~/code/gastown/internal/tmux/tmux.go` — read it, don't import it.
- **No cobra** — the CLI is a simple `switch` dispatch. 3 commands don't need a framework.
- **`make check`** runs fmt + vet + lint + test. Run before committing.
...
**Why this matters:** Every automated interaction in the athanor is a whisper. The marut nudging myrmidons, dispatch spawning crucibles, handoff chaining sessions — all depend on reliable message delivery to tmux panes. If whispers are flaky, everything built on top is flaky. `[B:domain-model.md § Infrastructure]`
