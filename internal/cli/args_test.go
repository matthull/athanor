...
func TestEnvSourcePrefix(t *testing.T) {
	t.Parallel()
	prefix := envSourcePrefix("/home/user/athanor/athanors/myath")
	// Must contain test -f guard and source command
	if !strings.Contains(prefix, "test -f /home/user/athanor/athanors/myath/.env.local") {
		t.Errorf("expected test -f guard, got: %s", prefix)
	}
	if !strings.Contains(prefix, ". /home/user/athanor/athanors/myath/.env.local") {
