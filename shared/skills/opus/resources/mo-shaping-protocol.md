...
The artifex's raw expression may be too narrow ("fix the field-mapping bugs") or too broad ("make the product better"). Your job is to find the altitude where:
...
- **Which concerns are hardest to verify?** The easy-to-automate fires happen naturally. The hard ones need explicit attention. "Domain credibility requires a human with real subject-matter expertise."
...
| Write this | Not this |
|---|---|
| "Domain credibility is the gate the subject-matter expert holds" | "Run eslint, jest, and pgTAP before merge" |
| "Emotional resonance is the highest-value fire and the hardest to automate" | "Use the mock-UAT and domain-critique skills as a pair" |
...
| Write this | Not this |
|---|---|
| "The artifex would feel confident putting this in front of the client" | "Demo must cover 5 workflows, have no errors, and include a walkthrough doc" |
...
The second column isn't wrong — those might be exactly what abundant satisfaction requires. But they should be *discovered* through operation, not *prescribed* in the MO. An agent that derives "we need a walkthrough doc" from reasoning about the client's experience is more reliable than one that checks "walkthrough doc" off a list, because the agent that reasons will also notice the ten other things that client needs that the list didn't mention.
