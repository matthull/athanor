...
| | Intent | Boundary |
|---|---|---|
| Good | The notification service handles nil recipients gracefully — no more NoMethodError alerts in the error channel | Agent: full (investigate, fix, verify, PR). Operator: review and merge. |
| Good | The team understands why the nightly CRM sync times out and has a path to fix it | Agent: full investigation. Operator: judgment call on approach if ambiguous. |
...
| Bad | Produce an inventory by: 1) audit the vendor's admin UI, 2) catalog interaction patterns for setup/validation/errors, 3) rate complexity, 4) create a ticket | *(intent mixed with procedure — the numbered steps belong in Context as suggestions, not in Goal)* |
| Good | The team understands the UX complexity landscape for the field-mapping admin screen — patterns needed, design risks, decisions required before implementation | *(same work, stated as witness experience delta)* |
...
Name files with a datestamp prefix: `YYYY-MM-DD-<descriptive-name>.md` (e.g., `2026-03-23-fix-nomethoderror-in-notification-service.md`). The datestamp is the date of inscription and never changes.
